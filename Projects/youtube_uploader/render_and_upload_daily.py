    ...
    while response is None:
        try:
            ...
            if e.resp.status in (500, 502, 503, 504):
                log(f"Transient HTTP {e.resp.status}")
                time.sleep(10)
                continue
            raise
    return response["id"]
    ...
    while True:
        ...
        try:
            ...
            if meta.get("hashtags"):
                title = f"{title} {meta['hashtags']}".strip() # title from .csv with hashtags (for shorts)
            ...
            log(f"Video ID: {vid}  URL: https://www.youtube.com/watch?v={vid}")
